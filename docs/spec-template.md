...
Two to three sentences on key design decisions — focus on *why*, not *what*.
The Purpose section covers what the subsystem does; this section explains
architectural choices not obvious from the code.
...
List only exports consumed outside this subsystem's directory. Internal helpers
...
Number sequentially. Focus on failures that have occurred or are likely, not
...
is covered by the nearest SPEC.md up the directory tree. For example,
`src/ragling/tools/` is documented in its parent `src/ragling/SPEC.md` under
"Key files." A directory needs its own SPEC.md only when it has independent
invariants and failure modes worth tracking separately.
